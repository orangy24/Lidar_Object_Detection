...
void Proximity(std::vector<float>& point, int id, std::vector<int>& cluster, KdTree* tree, float distanceTol) {
	visited.insert(id);
	cluster.push_back(id);
	std::vector<int> nearest = tree->search(point, distanceTol);
	for (int id : nearest) {
		if (visited.find(id) == visited.end()) {
			Proximity(point, id, cluster, tree, distanceTol);
		}
	}
}
std::vector<std::vector<int>> euclideanCluster(std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int i = 0; i < points.size(); i++) {
		if (visited.find(i) == visited.end()) {
			...
			Proximity(points[i], i, cluster, tree, distanceTol);
